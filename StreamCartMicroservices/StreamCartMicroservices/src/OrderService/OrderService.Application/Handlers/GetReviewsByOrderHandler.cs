...
{
    public class GetReviewsByOrderHandler : IRequestHandler<GetReviewsByOrderQuery, IEnumerable<ReviewDTO>>
    {
        ...
        public async Task<IEnumerable<ReviewDTO>> Handle(GetReviewsByOrderQuery request, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogInformation("Getting reviews for order {OrderId}", request.OrderId);

                // Lấy reviews từ repository
                var reviews = await _reviewRepository.GetByOrderIdAsync(request.OrderId);
                ...
                _logger.LogInformation("Successfully retrieved {Count} reviews for order {OrderId}", reviewDTOs.Count, request.OrderId);

                return reviewDTOs;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting reviews for order {OrderId}", request.OrderId);
                throw;
            }
        }
    }
