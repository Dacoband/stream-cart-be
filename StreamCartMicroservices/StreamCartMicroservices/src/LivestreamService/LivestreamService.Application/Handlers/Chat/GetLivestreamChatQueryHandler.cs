...
{
    public class GetLivestreamChatQueryHandler : IRequestHandler<GetLivestreamChatQuery, PagedResult<LivestreamChatDTO>>
    {
        private readonly ILivestreamChatRepository _chatRepository;
        private readonly ILogger<GetLivestreamChatQueryHandler> _logger;

        public GetLivestreamChatQueryHandler(
            ILivestreamChatRepository chatRepository,
            ILogger<GetLivestreamChatQueryHandler> logger)
        {
        ...
        public async Task<PagedResult<LivestreamChatDTO>> Handle(GetLivestreamChatQuery request, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogInformation("Getting livestream chat for livestream {LivestreamId}", request.LivestreamId);
            ...
            {
                _logger.LogError(ex, "Error getting livestream chat for livestream {LivestreamId}", request.LivestreamId);
                throw;
            }
        }
    }
