...
namespace LivestreamService.Application.Handlers
{
    public class GetActiveLivestreamsQueryHandler : IRequestHandler<GetActiveLivestreamsQuery, List<LivestreamDTO>>
    {
        ...
        public GetActiveLivestreamsQueryHandler(
        ...
        {
            var activeLivestreams = await _livestreamRepository.GetActiveLivestreamsAsync();

            if (request.IncludePromotedOnly)
            {
                activeLivestreams = activeLivestreams.Where(l => l.IsPromoted).ToList();
            }
